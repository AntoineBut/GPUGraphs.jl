...
@testset "mul!" begin
    ...
    gpu_spmv!(C_gpu_2, A_gpu_sell, B_gpu)
    KernelAbstractions.synchronize(TEST_BACKEND)
    copyto!(res, C_gpu_2)
    @test isapprox(res, C_cpu)
...
@testset "masked_mul!" begin
    ...
    """ Sparse mask is not fully supported yet.
    ...
    # SELL 
    C_gpu_2 = KernelAbstractions.zeros(TEST_BACKEND, Float32, 10)
    gpu_spmv!(C_gpu_2, A_gpu_sell, B_gpu; mask = mask_dense)
    KernelAbstractions.synchronize(TEST_BACKEND)
    copyto!(res, C_gpu_2)
    @allowscalar @test isapprox(res, C_cpu)

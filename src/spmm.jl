...
@kernel function csr_spmm_kernel!(
    ...
    for i = a_row_ptr[row]:a_row_ptr[row+1]-1
        ...
        col_A = a_col_val[i]
        acc = add(acc, mul(a_nz_val[i], B[col_A, col_B_C], row, col_A, col_A, col_B_C), row, col_A, col_A, col_B_C)
...
@kernel function sell_spmm_kernel!(
    ...
    acc = monoid_neutral_element
    if row <= n
        for i in a_slice_ptr[slice] + offset:a_slice_size:a_slice_ptr[slice + 1] - 1
